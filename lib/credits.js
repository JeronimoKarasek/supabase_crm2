    ...
    if (useDbFallback) {
      try {
        const { data, error } = await supabaseAdmin
          ...
          .single()
        if (error) return 0
        const n = Number(data?.credits_balance_cents || 0)
...
async function setBalanceCents(userId, cents, empresaId=null) {
  ...
  if (empresaId) {
    if (useDbFallback) {
      await supabaseAdmin.rpc('empresa_add_credits', { p_empresa: empresaId, p_cents: 0 })
      // set direto não existe; fazer via update para set absoluto
      await supabaseAdmin.from('empresa').update({ credits_balance_cents: finalCents, updated_at: new Date().toISOString() }).eq('id', empresaId)
      return
    }
