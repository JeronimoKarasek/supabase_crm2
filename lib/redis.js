...
// Log do modo ativo para debug
console.log(`[Redis] Modo ativo: ${mode}`)
if (mode === 'memory') {
  console.warn('[Redis] ⚠️ ATENÇÃO: Usando fallback em memória - dados serão perdidos ao reiniciar!')
  console.warn('[Redis] Configure UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN ou REDIS_URL para persistência')
}
...
async function incrby(key, by) {
  const delta = Number(by) || 0
  if (mode === 'upstash' && typeof client.incrby === 'function') return client.incrby(key, delta)
  if (mode === 'ioredis' && typeof client.incrby === 'function') return client.incrby(key, delta)
  const v = Number(memory.get(key) || 0) + delta
  memory.set(key, String(v))
  return v
}

async function decrby(key, by) {
  const delta = Number(by) || 0
  if (mode === 'upstash' && typeof client.decrby === 'function') return client.decrby(key, delta)
  if (mode === 'ioredis' && typeof client.decrby === 'function') return client.decrby(key, delta)
  const v = Number(memory.get(key) || 0) - delta
  memory.set(key, String(v))
  return v
}
