...
// Valida CPF (dígito verificador)
export function isValidCPF(cpf) {
  if (!cpf) return false
  const numericCpf = cpf.replace(/\D/g, '')
  if (numericCpf.length !== 11) return false
  if (/^(\d)\1{10}$/.test(numericCpf)) return false // 111.111.111-11 etc
...
// Retorna CPF válido: usa o fornecido se válido, senão usa padrão
export function getValidCPF(cpf) {
  const cleaned = (cpf || '').replace(/\D/g, '')
  if (cleaned.length === 11 && isValidCPF(cleaned)) {
    return cleaned
  }
  // CPF válido padrão para sandbox/testes: 11144477735
  return '11144477735'
}
