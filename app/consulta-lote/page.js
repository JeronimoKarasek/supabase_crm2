"u\r\n\r\n  const reconsult = async (id) => {\r\n    try {\r\n      const { data: sessionData } = await supabase.auth.getSession()\r\n      const token = sessionData?.session?.access_token\r\n      const res = await fetch('/api/importar', { method: 'PUT', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: Bearer  } : {}) }, body: JSON.stringify({ id }) })\r\n      const json = await res.json().catch(() => ({}))\r\n      if (!res.ok) throw new Error(json?.error || 'Falha ao acionar reconsulta')\r\n      setMessage('Re-Consulta acionada')\r\n      setTimeout(() => setMessage(''), 2000)\r\n    } catch (e) {\r\n      setMessage(e?.message || 'Erro ao re-consultar')\r\n      setTimeout(() => setMessage(''), 2500)\r\n    }\r\n  }\r\nse client"
...
export default function ConsultaLotePage() {
  ...
  // Removido boṭo de excluṣo; mantemos apenas exporta̤̣o
                  ...
                  {list.map((it) => (
                    <div key={it.id} className="p-2 border rounded flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="secondary" onClick={() => reconsult(it.id)}>Re-Consultar</Button>
                        {/* Boṭo Excluir removido conforme solicitado */}
                      </div>
                    </div>
                  ))}
                </div>
