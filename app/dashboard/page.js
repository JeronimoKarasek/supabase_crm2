...
const parseMoney = (val) => {
  ...
  if (!s) return null
  // Ignore values with letters (keep only numeric-like strings)
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
                  ...
                  <DialogContent className="max-w-5xl">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2 justify-end">
                        <Button onClick={createChart} variant="outline" disabled={!table || !xCol || (agg === 'sum' && !yCol) || loading}>{loading? 'Gerando...' : 'Pré-visualizar'}</Button>
                        <Button onClick={() => {
                          ...
                          setOpenCreate(false)
                        }} disabled={!table || !xCol || (agg === 'sum' && !yCol)}>Salvar gráfico</Button>
                      </div>
                          ...
                          {showTable && (
                            <div className="border rounded">
                              <Table>
                                ...
                                <TableBody>
                                  {data.map((r,idx)=>(<TableRow key={idx}><TableCell>{r.x}</TableCell><TableCell>{agg==='sum'? fmtBRL(r.y) : r.y}</TableCell></TableRow>))}
                                </TableBody>
                              </Table>
                            </div>
                          )}
            ...
            </div><div className="text-sm font-medium">Total Pago: <span className="font-semibold">{fmtBRL(valorPago)}</span></div>
            </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        {c.showTable && (
                          <div className="border rounded overflow-auto">
                            <Table>
                              <TableHeader>
                                <TableRow><TableHead>Categoria</TableHead><TableHead>Valor</TableHead></TableRow>
                              </TableHeader>
                              <TableBody>
                                {(chartDataMap[c.id] || []).map((r,idx)=>(
                                  <TableRow key={idx}><TableCell>{r.x}</TableCell><TableCell>{c.agg==='sum'? fmtBRL(r.y) : r.y}</TableCell></TableRow>
                                ))}
                              </TableBody>
                            </Table>
                          </div>
