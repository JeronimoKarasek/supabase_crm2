...
export default function SimularDigitarPage() {
  ...
  const [results, setResults] = useState([]) // [{bankKey, bankName, data|error}]
  ...
  const retryOne = async (bankKey) => {
    if (!cpf) { setMessage('Informe o CPF'); setTimeout(()=>setMessage(''), 2000); return }
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const token = sessionData?.session?.access_token
      const res = await fetch('/api/simular', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ cpf, bankKey }) })
      const json = await res.json()
      if (!res.ok) throw new Error(json?.error || 'Falha na simulação')
      const updated = results.map(r => (r.bankKey === bankKey ? (json.results?.[0] || r) : r))
      setResults(updated)
    } catch (e) {
      setMessage(e?.message || 'Erro ao consultar')
      setTimeout(()=>setMessage(''), 2500)
  ...
  const canDigit = (r) => {
    ...
    // fallback: try search in _raw keys (support spaces/accents)
    const src = d._raw || {}
    const norm = (s) => {
      try { return String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]/g,'') } catch { return String(s||'').toLowerCase() }
    }
  ...
  const doDigitar = async () => {
    ...
    try {
      const { data: sessionData } = await supabase.auth.getSession()
      const token = sessionData?.session?.access_token
      const res = await fetch('/api/digitar', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ bankKey: currentBank.key, cpf, payload: digForm }) })
      const json = await res.json()
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      <div className="container mx-auto py-8 px-4 space-y-4">
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>Resultado do simulador</CardDescription>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="outline" onClick={() => retryOne(r.bankKey)}>Tentar novamente</Button>
                      {canDigit(r) && <Button onClick={() => openDigitar(r)}>Digitar</Button>}
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  {r.error ? (
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {/* Fallback: render any additional keys returned by webhook */}
                      {(() => {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Digitar: {currentBank?.name}</DialogTitle>
