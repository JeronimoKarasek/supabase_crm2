...
export async function GET(request) {
  ...
  try {
    ...
    if (!credential_id) return NextResponse.json({ error: 'credential_id obrigat�rio' }, { status: 400 })
    ...
    // categorias pagas (para refletir o cart�o "Mensagens pagas entregues")
    const categories = { marketing: 0, service: 0, authentication: 0, authentication_international: 0, utility: 0 }
    // contadores de conversas gr�tis (para refletir o cart�o "Mensagens gr�tis entregues")
    ...
    async function fetchForPhone(pn) {
      ...
      // Fallback: WABA-level insights (messages metric) com dimens�es por n�mero
      try {
    ...
    try {
      ...
      if (none || hasEdgeErrors) {
        ...
        // Derivar pagas/gr�tis e categorias a partir de status_events (quando poss�vel)
        try {
    ...
    // entregue total (quando poss�vel, usar soma paid+free para ficar consistente com o Manager)
