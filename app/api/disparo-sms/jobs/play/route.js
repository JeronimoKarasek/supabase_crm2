...
export async function POST(request){
  ...
  try {
    ...
    const res = await fetch('https://weebserver6.farolchat.com/webhook/v1/sms/jobs/play', {
      method: 'POST',
      headers: {
    ...
    if(!res.ok){
      return NextResponse.json({ error: 'Falha ao retomar job', details: json }, { status: res.status })
    }
